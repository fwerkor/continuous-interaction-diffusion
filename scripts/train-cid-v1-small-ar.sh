...
case "$SIZE" in
  2b)
    DEFAULT_MODEL="openbmb/MiniCPM5-2B-Base"
    DEFAULT_RUN_NAME="CID-v1-2B"
    ;;
  4b)
    DEFAULT_MODEL="Qwen/Qwen3-4B-Base"
    DEFAULT_RUN_NAME="CID-v1-4B"
    ;;
  *)
    echo "usage: $0 {2b|4b} {stage-a|stage-b|all}" >&2
    exit 2
    ;;
esac
case "$MODE" in
...
BACKBONE_LR_SCALE="${BACKBONE_LR_SCALE:-0.5}"
TARGET_GLOBAL_BATCH_A="${TARGET_GLOBAL_BATCH_A:-96}"
TARGET_GLOBAL_BATCH_B="${TARGET_GLOBAL_BATCH_B:-32}"
...
run_stage_a() {
  if [[ -f "$STAGE_A_FINAL" ]]; then
    echo "$RUN_NAME Stage A already complete: $STAGE_A_FINAL"
    return 0
  fi
  ...
  run_cmd "$PYTHON" -m torch.distributed.run --standalone \
    --nproc-per-node="$NPROC_PER_NODE" -m cid.cli train \
    --data "$DATA" \
    ...
    --dtype bf16 \
    --epochs 3 \
    --learning-rate "$STAGE_A_LR" \
    --weight-decay 0.01 \
    --micro-batch-size 1 \
    --target-global-batch-size "$TARGET_GLOBAL_BATCH_A" \
...
run_stage_b() {
  if [[ -d "$STAGE_B_FINAL" ]]; then
    echo "$RUN_NAME Stage B already complete: $STAGE_B_FINAL"
    return 0
  fi
  ...
  run_cmd "$PYTHON" -m torch.distributed.run --standalone \
    --nproc-per-node="$NPROC_PER_NODE" -m cid.cli train-full \
    --data "$DATA" \
    --validation-data "$VALIDATION_DATA" \
    ...
    --dtype bf16 \
    --epochs 1 \
    --learning-rate "$STAGE_B_LR" \
    --backbone-lr-scale "$BACKBONE_LR_SCALE" \
    --weight-decay 0.01 \
    --micro-batch-size 1 \
    --target-global-batch-size "$TARGET_GLOBAL_BATCH_B" \
    --gradient-checkpointing \
